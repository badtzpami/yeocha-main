...
<table class="table table-hover" id="table_material_supplier">
    <thead>
        <tr>
            <th>#</th>
    ...
    <tbody id="input-container-archive">
        ...
        <?php
        $material_list_query = "SELECT * FROM `supplier_material` WHERE stock <= 30 AND `user_id` = " . $user['user_id'] . " ORDER BY `sm_id` DESC";
        ...
        if (mysqli_num_rows($material_list_result) > 0) {
            while ($material = mysqli_fetch_array($material_list_result)) { ?>
                ...
                <tr>
                    <input type="hidden" name="sm_id[]" class="form-control" readonly value="<?php echo $material["sm_id"] ?>">
                    ...
                    <td>
                        <div class="profile-image" style="justify-content: center; margin-left: 25px;">
                            <form class="form" id="formmaterial<?php echo $material["sm_id"] ?>">
                                <div class="upload" style=" width: 125px;
                                    ...
                                    <div class="round" style=" position: absolute;
                                        ...
                                        <input type="file" name="imagematerial" id="imagematerial<?php echo $material["sm_id"]; ?>" accept=".jpg, .jpeg, .png" style=" position: absolute;
                                        transform: scale(2);
                                    ...
                                    <div class="round1" style="position: absolute;
                                        ...
                                        text-align: top;
                                        overflow: hidden;">
                                        <button type="button" class="btn" data-toggle="modal" data-target="#exampleMaterialModal<?php echo $material["sm_id"] ?>" style="top:0; background: transparent; border:0;width: 42px; height: 42px;"><i class="bi bi-arrows-angle-expand" style="color: #000; font-size: 20px; border:0; background: #fff;"></i></button>
                                    </div>
                                </div>
                            </form>
                        </div>
                    ...
                    <td style="width: 200px;"><input type="text" name="selling_price[]" class="form-control" value="<?php echo $material["selling_price"] ?>"></td>
                    ...
                    <!-- <td scope="col" style="width: 200px;">
                        <div class="d-flex">
                            <button class="btn btn-dark btn-sm delete_material button3" data-bs-toggle="modal" data-bs-target="#changePassword" value="<?= $material['sm_id']; ?>" data-role="change_password" id="<?php echo $material['sm_id']; ?>"><i class='bi bi-trash'></i></button>
                        </div>
                    </td> -->
                </tr>
                ...
                <!-- Modal -->
                <div class="modal fade pb-5" id="exampleMaterialModal<?php echo $material["sm_id"] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleMaterialModalLabel" aria-hidden="true" style="background-color: rgba(0,0,0,0.7);">
