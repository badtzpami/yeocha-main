...
<table class="table table-hover" id="table_active">
    <thead>
        <tr>
            <th>#</th>
            <th>User ID</th>
    ...
    <tbody id="input-container-archive">
        ...
        $sessionId = $id; 
        $user_list_query = "SELECT * FROM `users` WHERE  `status` = 'ACTIVE' && `role` = 'Supplier' ORDER BY `user_id` DESC";
        ...
        if (mysqli_num_rows($user_list_result) > 0) {
            while ($row = mysqli_fetch_array($user_list_result)) { ?>
                ...
                <tr>
                    ...
                    <td scope="col">
                        <input type="text" name="store[]" class="form-control" value="<?php echo $row["store"] ?>">
                    </td>
                    <td>
                        <select name="role[]" id="role">
                            <option value="" disabled <?php echo empty($row["role"]) ? 'selected' : ''; ?>>Choose a role</option>
                            <option value="Supplier" <?php echo ($row["role"] === 'Supplier') ? 'selected' : ''; ?>>Supplier</option>
                        </select>
