                ...
                <div id="<?= $ca_name ?>Products" class="collapse">
                    <div class="card-body">
                        <?php if (isset($products[$ca_name]) && !empty($products[$ca_name]['materials'])): ?>
                            <?php foreach ($products[$ca_name]['materials'] as $product_name => $materials): ?>
                                <div class="card mb-2" onclick="showMaterials('<?= $ca_name ?>', '<?= htmlspecialchars($product_name) ?>')">
                                    <div class="card-body">
                                        <span class="chevron-icon" id="icon<?= htmlspecialchars($product_name) ?>"><i class="fas fa-chevron-down"></i></span> <?= htmlspecialchars(str_replace('_', ' ', $product_name)) ?>
                                    </div>
                                    <?php

                                    if (is_array($product_image)) { ?>
                                        <?php if (empty($product_image)) { ?>
                                            <img src="../assets/images/default_images/tea.jpeg" width=200 height=200 title="yeaocha_main">
                                        <?php } else { ?>
                                            <img src="../assets/images/product_images/<?php echo $product_image; ?>" width=200 height=200 title="<?php echo $image; ?>">
                                    <?php }
                                    }
                                    ?>
                                </div>
                        ...
                        <?php elseif (isset($products[$ca_name]['no_products'])): ?>
                            <p>No products available for this category. (ID: <?= htmlspecialchars($ca_id) ?>)</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        function showMaterials(category, product) {
            ...
            if (materialsDiv) {
                ...
                <img src="${materials.image ? '../assets/images/product_images/' + materials.image : '../assets/images/default_images/tea.jpeg'}"
                     ...
                     style="border-radius: 50%; margin-right: 55px; border: 3px solid #DCDCDC; height: 60px; width: 60px;">
            `;
